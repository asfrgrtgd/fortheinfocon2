...
# include "mazeSearchDef.h"

using AppScene = SceneManager<String, algorithmIntroduction>;
class breadthFirstSearch : public AppScene::Scene, public mazeSearchDef {
private:
	...
	std::queue<std::pair<int, int>>queWillAdd;
	std::vector<Rect>searchCandidateSquares;
	String detailStr = U"";
	const String detailStr1Key = U"breadthFirstSearch.detailStr1";
	const String detailStr2Key = U"breadthFirstSearch.detailStr2";
	bool flag1 = true;
	bool flag2 = false;
	bool flagEnd = false;
	int nowStep = 0;
...
public:
	breadthFirstSearch(const InitData& init) : IScene(init) {
		getData().str = algo_vis::SearchINIDataFromKey(getData().data_ini, U"breadthFirstSearch.str");
	...
	void algo() {
		String ret;
		if (nowStep == 0) {
			ret += U"スタート地点である(" + Format(sx + 1) + U"," + Format(sy + 1) + U")を探索候補としてキューに入れ、スタート地点からの距離を0とします";
			dist[sy][sx] = 0;
			que.push({ sx,sy });
			...
			bool end = false;
			ret += U"キューの先頭の要素は(" + Format(nowX + 1) + U"," + Format(nowY + 1) + U")なのでこのマスについて近傍を調べます、このマスから見て未探索な近傍のマスは";
			int tmp = 0;
			for (int i = 0; i < 4; i++) {
			...
			if (tmp == 0) {
				...
				ret.pop_back();
				ret += U"です、これらのマスは元のマスから1マス移動する事でたどり着けるので、距離は元の(" + Format(nowX + 1) + U", " + Format(nowY + 1) + U")の距離に+1した値です。\n";
				for (int i = 0; i < 4; i++) {
					int nextX = nowX + dx[i], nextY = nowY + dy[i];
					if (isOk(nextX, nextY)) {
						if (maze[nextY][nextX] != 1) {
							dist[nextY][nextX] = dist[nowY][nowX] + 1;
							queWillAdd.push({ nextX,nextY });
						}
			...
			ret += U"1ステップ前に探索した近傍のマスをキューに追加し、探索済みの先頭の要素を消します(近傍マスがない場合はキューの先頭を削除するだけになります)";
			que.pop();
