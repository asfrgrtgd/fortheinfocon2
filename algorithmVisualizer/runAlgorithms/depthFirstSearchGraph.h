...
class depthFirstSearchGraph : public AppScene::Scene, graphSearchDef {
private:
	...
	std::stack<int>sta;
	std::stack<int>staWillAdd;
	std::vector<Rect>searchCandidateSquares;
	std::vector<Rect>distSquares;
	const String detailStr1Key = U"depthFirstSearchGraph.detailStr";
	bool flag1 = true;
	bool flagEnd = false;
	int nowStep = 0;
...
public:
	depthFirstSearchGraph(const InitData& init) : IScene(init) {
		getData().str = algo_vis::SearchINIDataFromKey(getData().data_ini, U"depthFirstSearchGraph.str");
	...
	void algo() {
		String ret;
		if (nowStep == 0) {
			ret = U"スタート地点である頂点1を探索候補としてスタックに入れ、頂点1の距離を0とします";
			dist[startVertex] = 0;
			flag1 = false;
			sta.push(startVertex);
			...
			int nowVertex = sta.top();
			ret += U"スタックの一番上の要素は(" + Format(nowVertex + 1) + U")なのでこの頂点から移動可能な近傍の頂点を調べます、この頂点から見て未探索な近傍の頂点は";
			for (int i = 0; i < verticesNum; i++) {
			...
			ret += U"1ステップ前に探索した近傍の頂点をスタックに追加し、探索済みの先頭の要素を消します(近傍頂点がない場合はスタックの先頭を削除するだけになります)";
			sta.pop();
			int tmp = (int)staWillAdd.size();
			for (int i = 0; i < tmp; i++) {
